...
public sealed class TransPair
{
    ...
    /// <summary>
    ///     原文改过、译文还没跟着改时由维护脚本写上说明（玩家提交的重合部分会保留，所以这里也可能有内容）。
    /// </summary>
...
public sealed class TranslationTable
{
    ...
    /// <summary>
    ///     有多少条记录里有「原文改过、译文还没跟上」的字段（上游改动提醒用）。
    /// </summary>
    ...
    /// <summary>
    ///     按顺序枚举词表（写操作与枚举都走同一把锁，避免后台读取时撞上写入）。
    /// </summary>
    ...
    private void BumpRevision() => Interlocked.Increment(ref revision);

    public bool TryGet(string internalName, out TransEntry entry)
    {
        lock (gate)
        {
            return table.TryGetValue(internalName, out entry!);
        }
    }

    /// <summary>
    ///     加载词表；返回是否至少加载到一份非空词表。
    /// </summary>
    /// <summary>
    ...
    public bool Load(out string? error)
    {
        ...
        foreach (var candidate in new[]
                 {
                     Path.Combine(configDirectory, "translations.json"),
                     Path.Combine(pluginDirectory, "translations.json"),
                 })
        {
            try
            {
        ...
        table = best.Table;
        MaintainedAt = best.MaintainedAt;
        LoadedFrom = best.Path;
        LoadedAt = File.GetLastWriteTime(best.Path);
        BumpRevision();
        return true;
    }
    ...
    public bool MarkUserTranslation(string internalName, string field, string original, string translated, string? note = null)
    {
        ...
        lock (gate)
        {
            return MarkUserTranslationCore(internalName, field, original, translated, note);
        }
    }
    ...
    private bool MarkUserTranslationCore(string internalName, string field, string original, string translated, string? note)
    {
        ...
        if (!table.TryGetValue(internalName, out var entry))
        {
            entry = new TransEntry();
            table[internalName] = entry;
        }
    ...
    /// <summary>
    ///     清掉某个插件的「待复核」标记（原文改过、但玩家已重新提交时用）。
    /// </summary>
    public void ClearReview(string internalName, string field)
    {
        lock (gate)
        {
            ClearReviewCore(internalName, field);
        }
    }
    ...
    /// <summary>
    ///     把当前内存里的词表写回配置目录（贡献提交后调用，让改动能落盘）。
    /// </summary>
    public bool SaveToConfigDirectory(out string? error)
    {
        ...
        try
        {
            ...
            lock (gate)
            {
                ...
                var payload = new Dictionary<string, object?>(table.Count + 1, StringComparer.Ordinal);
                if (!string.IsNullOrEmpty(MaintainedAt))
                {
                    payload[MetaKey] = new Dictionary<string, string> { ["updatedAt"] = MaintainedAt! };
                }
    ...
    /// <summary>
    ///     解析一份词表：以下划线开口的键走元数据（目前只有 <c>_meta.updatedAt</c>），其余当插件条目。
    /// </summary>
    ...
    /// <summary>
    ///     从 GitHub 拉取最新词表（失败会依次尝试镜像），成功后写入插件配置目录。
    /// </summary>
    public async Task<(bool Ok, string Message)> UpdateFromGitHubAsync(CancellationToken cancellationToken)
    {
        ...
        foreach (var url in new[] { RemoteURL, MirrorAtmoomen, MirrorGhProxy })
        {
            try
            {
                ...
                var localDate = DateTime.TryParse(MaintainedAt, out var local) ? local : (DateTime?)null;
