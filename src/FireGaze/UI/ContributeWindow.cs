...
internal sealed class ContributeWindow : Window
{
    ...
    /// <summary>提交分两步：点「一键提交」只打开 GitHub 提交页；玩家在网页按过 Submit 回来点「确认已提交」才清空待提交。</summary>
    ...
    private void SubmitContributions()
    {
        ...
        try
        {
            ...
            this.SetStatus($"已打开 GitHub 提交页（{count} 条）：在网页上按 Submit，回来点「确认已提交」", isError: false);
        }
        catch (Exception e)
        {
    ...
    /// <summary>一键提交（第二步）：玩家在网页按过 Submit 之后才归档 + 清空。</summary>
    private void ConfirmSubmitted()
    {
        ...
        this.workspaceTab = 0;   // 切到「历史提交」，让玩家看到刚才那一批
        this.selectedRecords.Clear();
        this.awaitingConfirm = false;
        this.SetStatus($"已提交 {count} 条：GitHub 上那条 issue 里能看到，维护者收录后会进词表", isError: false);
    }
